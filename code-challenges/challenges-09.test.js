...
myNewArray.push(`<li>${key}: ${value}</li>`);
...
let myTotal=arr.reduce((acc,values)=>{
...
const countNumberOfElements = (arr) => {
  return arr.reduce(accumator => accumator += 1,0);
};
...
const returnNames = (arr) => {
 return arr.reduce((acc,cun,indx)=>{
 acc[indx]=cun.name;
return acc;
 },[])
};
